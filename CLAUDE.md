...
- `theme2-biotech.html` — "Digital Pulse" (dark teal + cyan on #0f172a, Plus Jakarta Sans, glassmorphism, particles)
...
- `theme5-bioluminescent.html` — "Living Light" (neon green #00ff87 + magenta on #06060f, Syne + Outfit, aurora waves, cursor glow)
...
- Grid icon links back to `index.html`, 5 split-gradient dots link to each theme
- Active theme dot is scaled up with a glowing ring; hover shows tooltip with theme name
- Styled natively per theme (e.g., brutalist gets offset shadow + monospace tips, bioluminescent gets neon glow, warm gets bouncy spring easing)
...
- External image dependencies: theme2 still uses `ui-avatars.com` for team avatars
...
- Section IDs available across all themes: `services`, `about`, `testimonials`, `team`, `contact`. Only theme2 and theme5 have `id="hero"`; only theme2 has `id="stats"`.
- Theme4 has `*, *::before, *::after { border-radius: 0 !important; }` — any new elements inherit square corners
- Noise/grain overlays in theme1, theme4, theme5 use `z-index: 9999` with `pointer-events: none` — new floating UI needs `z-index: 10001+`
...
- **Theme2**: `md:` breakpoints for stats (`md:grid-cols-4`), services (`md:grid-cols-2`), about (`md:grid-cols-5`), contact (`md:grid-cols-2`)
- **Theme3**: Already had good `md:` coverage — no gaps
- **Theme4**: Already had good `md:` coverage — no gaps
- **Theme5**: `md:` breakpoints for about (`md:grid-cols-5`), contact (`md:grid-cols-2`), heading sizes `text-2xl → sm:3xl → md:4xl → lg:5xl`
